...
**Specification:** Read `asset-manifest.json`, the gallery `index.html`, `SOURCES.md`, `source/study-data.json`, `EDITORIAL.md`, `PILOT-COPY.md` and `COPY-STANDARD.md` in this package. The manifest is authoritative for figure text, data, dimensions and placement; the source ledger records the checked paper facts and supporting locations. Version 1.1 changes some figure heights, so use the current manifest. The gallery demonstrates the assets; it is not application verification. Work through the three implementation tasks below in order. This is a handoff to the development team, not authorization to merge or publish without the owner's normal review.
...
- [ ] Apply the ready-to-use passages in `PILOT-COPY.md`, following `COPY-STANDARD.md`. Find the six existing anchor paragraphs before replacing their text. A figure's old placement prefix is a migration instruction, not a requirement to retain unclear copy. Ensure the revised article and its figure agree; preserve all study quotations verbatim.
