  ...
  return (
    <MarketingShell>
      ...
      <section className="py-10">
        <h1 className="font-display font-extrabold text-[2.2rem] leading-tight text-ink sm:text-[2.6rem]">
        ...
        <ul className="mt-8 grid gap-3 sm:grid-cols-2">
          {BLOG_POSTS.map((post) => (
            <li key={post.slug}>
              <Link href={`/blog/${post.slug}`} className="surface block h-full px-5 py-5 transition-colors hover:border-ink">
                <span className="font-display font-extrabold text-lg leading-snug text-ink">{post.title}</span>
                <span className="mt-2 block text-sm leading-relaxed text-muted">{post.metaDescription}</span>
              </Link>
            </li>
          ))}
        </ul>
