...
function shell(inner: string): string {
  return `<div style="font-family:ui-sans-serif,system-ui,sans-serif;background:#F1F3F6;color:#1A2233;padding:32px 20px">
  <div style="max-width:420px;margin:0 auto">${inner}</div>
</div>`;
}

function button(href: string, label: string): string {
  return `<a href="${href}" style="display:inline-block;background:#2438E8;color:#fff;text-decoration:none;font-weight:600;padding:14px 22px;border-radius:14px">${escapeHtml(label)}</a>`;
}
...
export function reminderEmail(input: {
  ...
  const html = shell(`
    ...
    <p style="font-size:13px;color:#5B6475;margin:24px 0 0">Not useful? Turn reminders off under You.</p>`);
...
export function magicLinkEmail(input: { link: string; minutes: number }): {
  ...
  const html = shell(`
    ...
    <p style="font-size:15px;line-height:1.5;color:#3D4658;margin:0 0 20px">This link signs you in on any device. It works once and expires in ${input.minutes} minutes.</p>
    ${button(input.link, "Sign in to DeskBreak")}
    <p style="font-size:13px;color:#5B6475;margin:24px 0 0">If you didn't ask for this, you can ignore it.</p>`);
