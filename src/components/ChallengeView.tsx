...
export function ChallengeView() {
  ...
  if (!challenge.startedOn) {
    return (
      <div className="flex flex-1 flex-col px-5 py-6 lg:max-w-[560px] lg:px-0">
        ...
        <h1 className="mt-4 font-display font-extrabold text-[1.8rem] leading-tight text-ink">
        ...
        <fieldset className="mt-7">
          <legend className="font-display font-extrabold text-lg text-ink">
            ...
            {RATINGS.map((rating) => (
              <button
                ...
                className={[
                  ...
                  baseline === rating.id ? "bg-ink text-paper" : "border border-line-strong bg-sheet text-ink hover:border-ink",
                ].join(" ")}
              >
                <span className={["w-5 text-sm tabular-nums", baseline === rating.id ? "text-paper/60" : "text-muted"].join(" ")}>
                  {rating.id}
                </span>
                {rating.label}
              </button>
  ...
  if (challenge.completedAt && challenge.finalRating) {
    return (
      <div className="flex flex-1 flex-col px-5 py-6 lg:max-w-[560px] lg:px-0">
        ...
        <h1 className="mt-4 font-display font-extrabold text-[1.8rem] leading-tight text-ink">Your DeskBreak week</h1>

        <div className="surface mt-5 grid grid-cols-2 gap-4 px-4 py-5">
          <div>
            <p className="text-sm font-semibold text-muted">Day 1</p>
            <p className="mt-1 font-display font-extrabold text-lg text-ink">
              {challenge.baseline ? RATING_LABEL[challenge.baseline] : "–"}
            </p>
          </div>
          <div>
            <p className="text-sm font-semibold text-muted">Day 5</p>
            <p className="mt-1 font-display font-extrabold text-lg text-ink">{RATING_LABEL[challenge.finalRating]}</p>
          </div>
        </div>

        <p className="mt-5 font-display font-extrabold text-xl text-ink">
  ...
  return (
    <div className="flex flex-1 flex-col px-5 py-6 lg:max-w-[560px] lg:px-0">
      <p className="text-sm font-semibold text-pen">5-Day Desk Reset</p>
      <h1 className="mt-1 font-display font-extrabold text-[1.8rem] leading-tight text-ink">
          ...
          return (
            <li
              ...
              className={[
                ...
                done ? "bg-note text-ink" : "surface text-muted",
              ].join(" ")}
          ...
          <div className="mt-3 grid gap-2">
            {RATINGS.map((rating) => (
              <button
                ...
                className={[
                  "flex min-h-13 items-center gap-3 rounded-[14px] px-4 text-left font-semibold transition-colors",
                  finalRating === rating.id ? "bg-ink text-paper" : "border border-line-strong bg-sheet text-ink hover:border-ink",
                ].join(" ")}
              >
                <span className={["w-5 text-sm tabular-nums", finalRating === rating.id ? "text-paper/60" : "text-muted"].join(" ")}>
                  {rating.id}
                </span>
                {rating.label}
              </button>
